...
export function materializeCoverElements(config: Partial<PdfConfig>): CoverElement[] {
  const base = (config.coverElements?.length ? config.coverElements : cloneDefaultCoverElements()).map((element) => ({
    ...element,
    items: element.items ? [...element.items] : undefined,
  }));
...
export const pdfConfigApi = {
  getConfig: (): Promise<PdfConfig> =>
    api.get<PdfConfig>('/checkup/pdf-config'),

  updateConfig: (config: PdfConfig): Promise<PdfConfig> =>
    api.put<PdfConfig>('/checkup/pdf-config', config),

  /** Invia l'HTML generato dal frontend e riceve il PDF come Blob */
  previewPdf: (html: string): Promise<Blob> =>
    requestBlob('/checkup/pdf-config/preview', {
      method: 'POST',
