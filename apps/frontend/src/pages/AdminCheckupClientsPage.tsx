...
import { checkupAdminApi, type CheckupStudio, type CheckupSublicense } from '../api/checkupAdmin';
...
export default function AdminCheckupClientsPage() {
  ...
  const [clients, setClients] = useState<CheckupStudio[]>([]);
  const [sublicenses, setSublicenses] = useState<CheckupSublicense[]>([]);
  const [loading, setLoading] = useState(false);
  const [showModal, setShowModal] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedClient, setSelectedClient] = useState<CheckupStudio | null>(null);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [studiosData, sublicensesData] = await Promise.all([
        checkupAdminApi.getStudios(),
        checkupAdminApi.getSublicenses(),
      ]);
      setClients(studiosData.filter((s) => s.tipo === 'cliente'));
  ...
  const sublicensesByClient = useMemo(() => {
    ...
    sublicenses.forEach((s) => {
      if (s.clienteStudioId) {
        map.set(s.clienteStudioId, s);
      }
    });
    return map;
  }, [sublicenses]);
  ...
  const availableSublicenses = useMemo(() => {
    return sublicenses.filter((s) => !s.clienteStudioId || s.clienteStudioId === selectedClient?.id);
  ...
  const handleOpenEdit = (client: CheckupStudio) => {
    setIsEditing(true);
    setSelectedClient(client);
    setSelectedSublicenseId(sublicensesByClient.get(client.id)?.id || '');
    setFormData({
      nome: client.nome,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let client: CheckupStudio;
      if (isEditing && selectedClient) {
        client = await checkupAdminApi.updateStudio(selectedClient.id, {
        ...
        client = await checkupAdminApi.createStudio({
          nome: formData.nome.trim(),
          tipo: 'cliente',
          ragioneSociale: formData.ragioneSociale.trim(),
          partitaIva: formData.partitaIva.trim(),
  ...
  const handleToggleActive = async (client: CheckupStudio) => {
    ...
    try {
      if (client.attivo) {
        await checkupAdminApi.deactivateStudio(client.id);
        success('Cliente disattivato');
      } else {
        await checkupAdminApi.updateStudio(client.id, { attivo: true });
